...
class PackageGenerator:
    def __init__(self, jsonfile):
        ...
        self.startmenu_shortcut = (
            self.basename + ".exe"
        )  # jsondata.get("startmenu_shortcut", None)
        self.desktop_shortcut = (
            self.basename + ".exe"
        )  # jsondata.get("desktop_shortcut", None)
    ...
    def generate_files(self):
        ...
        if self.major_upgrade is not None:
            ...
            ET.SubElement(
                ...
                "MajorUpgrade",
                {
                    "DowngradeErrorMessage": "A newer version of %s is already installed."
                    % self.name
                },
            )
        if self.arch == 64:
        ...
        if self.startmenu_shortcut is not None:
            ap = ET.SubElement(
                product, "DirectoryRef", {"Id": "ApplicationProgramsFolder"}
            )
            comp = ET.SubElement(
                ap,
                "Component",
        ...
        if self.desktop_shortcut is not None:
            ET.SubElement(
                top_feature, "ComponentRef", {"Id": "ApplicationShortcutDesktop"}
            )
        ...
        if self.registry_entries is not None:
            registry_entries_directory = ET.SubElement(
                product, "DirectoryRef", {"Id": "TARGETDIR"}
            )
            registry_entries_component = ET.SubElement(
                registry_entries_directory,
                "Component",
    ...
    def create_xml(self, nodes, current_dir, parent_xml_node, staging_dir):
        ...
        for dirname in cur_node.dirs:
            dir_id = (
                os.path.join(current_dir, dirname).replace("\\", "_").replace("/", "_")
            )
    ...
    def build_package(self):
        ...
        if platform.system() == "Windows":
            ...
            subprocess.check_call(
                [os.path.join(wixdir, "wixl"), "-o", self.final_output, self.main_xml]
            )
